...
void PreferencesDialog::accept() {
  ...
  // Check if all the default
  if (olive::CurrentConfig.default_transition_length != default_transition_length_field->value()) {
	  olive::CurrentConfig.default_transition_length = default_transition_length_field->value();
  }
...
void PreferencesDialog::setup_ui() {
  ...
  // Default
  QWidget* default_tab = new QWidget(this);
  tabWidget->addTab(default_tab, tr("Default"));

  row = 0;

  QGridLayout* default_layout = new QGridLayout(default_tab);

  // Default -> Transition
  default_layout->addWidget(new QLabel(tr("Transition:"), this), row, 0);

  default_transition_length_field = new QSpinBox(this);
