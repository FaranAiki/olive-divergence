...
void ExportDialog::setup_ui() {
  ...
  preferenceGroupbox = new QGroupBox(this);
  preferenceGroupbox->setTitle(tr("Preferences"));
  preferenceGroupbox->setFlat(false);
  preferenceGroupbox->setCheckable(true);
  
  QGridLayout* preferenceLayout = new QGridLayout(preferenceGroupbox);
  preferenceLayout->addWidget(new QLabel(tr("Shutdown After Exporting:"), this), 2, 0, 1, 1);
  shutdown_after_checkbox = new QCheckBox(this);
  preferenceLayout->addWidget(shutdown_after_checkbox, 2, 1, 1, 1);
  
  verticalLayout->addWidget(preferenceGroupbox);
